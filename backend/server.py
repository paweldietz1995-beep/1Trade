...
class BotSettings(BaseModel):
    ...
    protect_at_percent: float = 100.0
    protected_stop_percent: float = 40.0
    ...
    while auto_trading_state["is_running"]:
        ...
        try:
            ...
            for opp in opportunities:
                # Check if we've filled all available slots
                if trades_executed_this_cycle >= available_slots:
                    logger.info(f"✅ All slots filled ({trades_executed_this_cycle}/{available_slots})")
                    break
                ...
                try:
                    ...
                    if current_token_trades >= max_trades_per_token:
                    ...
                    # Update token trade count
                    token_trade_counts[token_address] = current_token_trades + 1
                    active_trade_tokens.add(token_address)
                    
                    # Set cooldown for this token (20 seconds)
                    set_signal_cooldown(token_address)
                    ...
                    # TRADE EXECUTED LOG
                    logger.info(f"✅ TRADE #{trades_executed_this_cycle} | {opp['symbol']} | {trade_amount:.4f} SOL | score={opp['signal_score']:.0f}")
            ...
            remaining_opps = opportunities[trades_executed_this_cycle:trades_executed_this_cycle + 50]  # Queue up to 50
            ...
            logger.info(f"⏱️ Cycle #{auto_trading_state['scan_count']} complete: {trades_executed_this_cycle} trades in {cycle_time:.2f}s | Total open: {final_open_trades}")
...
class MultiSourceScanner:
    ...
    async def scan_all_sources(self) -> List[Dict]:
        ...
        # ===== DETAILED SCANNER DEBUG =====
        logger.info("=" * 70)
        logger.info("🔍 SCANNER DEBUG - TOKENS BY SOURCE")
        for source_name, count in source_results.items():
            logger.info(f"   {source_name}: {count}")
        logger.info(f"   ─────────────────")
        logger.info(f"   tokens_total: {len(enriched_tokens)}")
        logger.info(f"   duplicates_removed: {len(all_tokens) - len(enriched_tokens)}")
        logger.info("=" * 70)
    ...
    async def scan_dexscreener(self) -> List[Dict]:
        """
        OPTIMIZED DexScreener Scanner with Rate-Limit Protection
        
        Features:
        - Strategic search queries for maximum coverage
        - Rate-limit protection with delays
        - Target: 500+ tokens per scan
        """
        pairs = []
        seen_addresses = set()
        ...
        try:
            async with httpx.AsyncClient(timeout=20.0) as client:
                # Strategic queries (reduced to avoid rate limits)
                queries = [
                    "solana", "sol meme", "pump.fun", "raydium",
                    "orca", "meteora", "bonk", "wif", "jup",
                    "sol degen", "meme", "solana token", "sol new"
                ]
                
                # Process queries in small batches with delays
                batch_size = 4
                    ...
                    async def fetch_query(q):
                        try:
                            ...
                            if resp.status_code == 200:
                                ...
                                await asyncio.sleep(1.0)  # Longer delay on rate limit
                        except:
                            pass
                        return []
    ...
    async def scan_birdeye(self) -> List[Dict]:
        """Optimized Birdeye-style Scanner"""
        pairs = []
        seen_addresses = set()
        
        try:
            async with httpx.AsyncClient(timeout=15.0) as client:
                queries = ["trending solana", "volume sol", "gainers sol", "hot solana"]
                
                async def fetch(q):
                    try:
                        resp = await client.get("https://api.dexscreener.com/latest/dex/search", params={"q": q})
                        if resp.status_code == 200:
                            return resp.json().get("pairs", [])
                    except:
                        pass
    ...
    async def scan_jupiter(self) -> List[Dict]:
        """Optimized Jupiter Scanner - uses token list"""
        pairs = []
        seen_addresses = set()
        ...
        try:
            async with httpx.AsyncClient(timeout=20.0) as client:
                ...
                if resp.status_code == 200:
                    tokens = resp.json()
                    for t in tokens[:500]:  # Top 500 verified tokens
                        addr = t.get("address", "")
                        if addr and addr not in seen_addresses:
                            pairs.append({
                                "chainId": "solana",
                                "baseToken": {"address": addr, "symbol": t.get("symbol", ""), "name": t.get("name", "")},
                                "priceUsd": "0",
                                "liquidity": {"usd": 5000},
    ...
    async def scan_raydium_pools(self) -> List[Dict]:
        """Optimized Raydium Scanner"""
        pairs = []
        seen_addresses = set()
        
        try:
            async with httpx.AsyncClient(timeout=15.0) as client:
                queries = ["raydium", "raydium sol", "raydium meme", "ray pool"]
    ...
    async def scan_orca_pools(self) -> List[Dict]:
        """Optimized Orca Scanner"""
        pairs = []
        seen_addresses = set()
        
        try:
            async with httpx.AsyncClient(timeout=15.0) as client:
                queries = ["orca", "orca sol", "whirlpool", "orca meme"]
    ...
    async def scan_meteora_pools(self) -> List[Dict]:
        """Optimized Meteora Scanner"""
        pairs = []
        seen_addresses = set()
        
        try:
            async with httpx.AsyncClient(timeout=15.0) as client:
                queries = ["meteora", "meteora sol", "dlmm", "meteora pool"]
    ...
    async def scan_pumpfun_pairs(self) -> List[Dict]:
        """Optimized Pump.fun Scanner"""
        pairs = []
        seen_addresses = set()
        
        try:
            async with httpx.AsyncClient(timeout=15.0) as client:
                queries = ["pump.fun", "pumpfun", "pump sol", "bonding curve", "pump meme"]
...
# Global scanner instance - Nutzt neuen MultiSourceScannerV4
# Der alte Code bleibt für Rückwärtskompatibilität als Fallback
multi_source_scanner = scanner_instance  # Verweis auf neuen Scanner
...
async def update_all_trade_prices():
    """
    HIGH CAPACITY: Bulk update prices for up to 120 trades.
    ...
    - Hard Stop Loss -12%
    """
    # ⚠️ ERHÖHT: Bis zu 150 Trades unterstützen
    open_trades = await db.trades.find({"status": "OPEN"}, {"_id": 0}).to_list(150)
...
async def get_strategy_config():
    ...
    Optimiert für: Große Gewinne, kleine Verluste + High Capacity Scaling
...
def calculate_dynamic_trade_size(wallet_balance: float, open_trades_count: int) -> dict:
    """
    Berechnet die dynamische Trade-Größe basierend auf Wallet-Balance.
    
    Formel: trade_size = available_balance / target_active_trades
    ...
    """
    target_trades = ENGINE_CONFIG.get("target_active_trades", 75)
    min_trade = ENGINE_CONFIG.get("min_trade_sol", 0.02)
    max_trade = ENGINE_CONFIG.get("max_trade_sol", 0.08)
    max_capital_percent = ENGINE_CONFIG.get("max_capital_in_trades_percent", 80)
    
    # Calculate available capital
    available_capital = wallet_balance * (max_capital_percent / 100)
    
    # Calculate remaining slots
    max_trades = ENGINE_CONFIG.get("max_open_trades", 100)
    remaining_slots = max(0, max_trades - open_trades_count)
    
    if remaining_slots == 0:
        return {
            "trade_size_sol": 0,
            "is_min_size": False,
            "warning": "Max trades reached",
            "can_trade": False
        }
    
    # Dynamic sizing: divide available capital by target trades
    effective_target = min(target_trades, remaining_slots)
    calculated_size = available_capital / max(effective_target, 1)
    
    # Apply limits
    trade_size = max(min_trade, min(calculated_size, max_trade))
    
    # Check if at minimum
    is_min_size = trade_size <= min_trade
    warning = None
    
    if is_min_size and calculated_size < min_trade:
        warning = f"Trade-Größe unter Minimum ({calculated_size:.4f} < {min_trade})"
    
    return {
        "trade_size_sol": round(trade_size, 4),
        "calculated_size": round(calculated_size, 4),
        "is_min_size": is_min_size,
        "warning": warning,
        "can_trade": True
    }
...
async def get_capital_status():
    """
    Gibt den aktuellen Kapitalstatus zurück.
    
    Returns:
        - available_balance: Freies Kapital
        - capital_in_trades: Gebundenes Kapital
        - capital_usage_percent: Auslastung in %
        - active_trades: Anzahl aktiver Trades
        - trade_capacity: Verbleibende Trade-Slots
        - dynamic_trade_size: Empfohlene Trade-Größe
    """
    # Get wallet balance
    settings = await get_bot_settings()
    ...
    # Get open trades
    open_trades = await db.trades.find({"status": "OPEN"}, {"_id": 0}).to_list(200)
    active_count = len(open_trades)
    
    # Calculate capital in trades
    capital_in_trades = sum(t.get("amount_sol", 0) * (t.get("remaining_percent", 100) / 100) for t in open_trades)
    
    # Calculate available balance
    available_balance = wallet_balance - capital_in_trades
    
    # Calculate usage
    capital_usage = (capital_in_trades / max(wallet_balance, 0.001)) * 100
    max_capital_percent = ENGINE_CONFIG.get("max_capital_in_trades_percent", 80)
    
    # Trade capacity
    max_trades = ENGINE_CONFIG.get("max_open_trades", 100)
    remaining_slots = max(0, max_trades - active_count)
    
    # Dynamic trade size
    trade_sizing = calculate_dynamic_trade_size(wallet_balance, active_count)
    
    # Warnings
    warnings = []
    if capital_usage >= max_capital_percent:
        warnings.append(f"Kapital-Limit erreicht ({capital_usage:.1f}%)")
    if remaining_slots == 0:
        warnings.append("Max Trades erreicht")
    if trade_sizing.get("warning"):
        warnings.append(trade_sizing["warning"])
    
    return {
        "wallet_balance": round(wallet_balance, 4),
        "available_balance": round(available_balance, 4),
        "capital_in_trades": round(capital_in_trades, 4),
        "capital_usage_percent": round(capital_usage, 1),
        ...
        "dynamic_trade_size": trade_sizing["trade_size_sol"],
        "min_trade_size": ENGINE_CONFIG.get("min_trade_sol", 0.02),
        "max_trade_size": ENGINE_CONFIG.get("max_trade_sol", 0.08),
        
        "avg_trade_size": round(capital_in_trades / max(active_count, 1), 4),
        
        "can_open_trades": remaining_slots > 0 and capital_usage < max_capital_percent,
        "warnings": warnings,
        
        "scaling": {
            "description": f"Mit {wallet_balance:.2f} SOL → ~{int(wallet_balance / trade_sizing['trade_size_sol'])} mögliche Trades",
            "recommended_trades": min(int(wallet_balance / trade_sizing["trade_size_sol"]), max_trades)
        }
    }


@api_router.get("/capital/metrics")
async def get_capital_metrics():
    """
    Gibt erweiterte Kapital-Metriken für das Dashboard zurück.
    """
    # Get basic status
    status = await get_capital_status()
    
    # Get trade history for metrics
    trades_today = await db.trades.find({
        "opened_at": {"$gte": datetime.now(timezone.utc).replace(hour=0, minute=0, second=0).isoformat()}
    }, {"_id": 0}).to_list(500)
    
    # Calculate trades per minute (last 10 minutes)
    ten_min_ago = datetime.now(timezone.utc) - timedelta(minutes=10)
    recent_trades = [t for t in trades_today if t.get("opened_at", "") > ten_min_ago.isoformat()]
    trades_per_minute = len(recent_trades) / 10 if recent_trades else 0
...
async def get_scanner_stats():
    ...
    - api_health: Rate limiting and backoff status
    """
    # Get comprehensive stats from new scanner
    scanner_stats = multi_source_scanner.get_stats()
    health_summary = scanner_health.get_health_summary()
    
    return {
        "scanner_version": "v4",  # Updated to V4
        "stats": scanner_stats,
        ...
        "config": {
            ...
            "target_tokens_per_cycle": "2000-6000",
            "target_scan_interval": "1-3s"
        }
    }
...
async def clear_scanner_cache():
    ...
    await multi_source_scanner.clear_cache()
    return {"success": True, "message": "Scanner cache cleared"}


@api_router.post("/scanner/reset-health")
async def reset_scanner_health(api_name: str = None):
    """
    Reset API health status to recover from rate-limiting.
    
    Args:
        api_name: Optional specific API to reset (dexscreener, birdeye, etc.)
                  If not provided, resets all APIs.
    """
    multi_source_scanner.reset_api_health(api_name)
    return {
        "success": True,
...
async def get_scanner_health():
    ...
    - scan_time
    """
    return scanner_health.get_health_summary()
